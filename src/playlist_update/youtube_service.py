...
def fetch_all_playlist_items(youtube, playlist_id):
    ...
    try:
        ...
        print(f"[API ERROR] Échec de récupération de la playlist : {e}")
        return []
...
def get_channel_uploads_id(youtube, channel_id):
    ...
    try:
        ...
        print(f"[API ERROR] Impossible de récupérer les détails de la chaîne : {e}")
        return None

def get_new_videos_from_channel(youtube, uploads_playlist_id, last_sync_date_iso):
    """
    Récupère les vidéos de la chaîne publiées strictement APRÈS last_sync_date_iso.
    Filtre: Ne garde que les vidéos dont la durée est > 60 secondes (exclut les Shorts).
    ...
    print(f"[SYNC] Recherche de vidéos publiées après {ref_date}...")
    ...
    try:
        while True:
            ...
            # Récupération des durées via l'API videos
            durations = {}  # {video_id: (formatted, seconds)}
            if video_ids:
                videos_response = youtube.videos().list(
                    part="contentDetails",
                    id=",".join(video_ids)
        ...
        print(f"[API ERROR] Erreur lors du scan des nouvelles vidéos : {e}")
        return []
...
def add_video_to_playlist(youtube, playlist_id, video_id):
    ...
    try:
        ...
        if "videoAlreadyInPlaylist" in error_details or e.resp.status == 409:
            print(f"[INFO] La vidéo {video_id} est déjà dans la playlist.")
            return True
        print(f"[API ERROR] Échec de l'ajout de la vidéo {video_id} : {e}")
        return False
